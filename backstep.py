...
if __name__ == '__main__':
    ...
    inp = -k_2 * z - (x_1 * g + h - phi_dot * (f + g * x_2)) # Controlling input
    z_dot = h + u - phi_dot * x_1dot * (f + g * x_2)
    ...
    plt_texts.append('$\dot{x_1} = %s$' % latex(simplify(f + g * phi + g * (x_2 - phi))))
    plt_texts.append('$\dot{z} = h(x_2) + u - \partial\phi(x_1)/\partial x_1(f(x_1) +g(x_1)x_2)$')
    plt_texts.append('$\dot{z} = %s$\n' % latex(simplify(z_dot)))
    plt_texts.append('$u(x_1, x_2) = %s$' % latex(simplify(inp)))
    plt_texts.append('$u(x_1, x_2) = %s$\n' % latex(simplify(inp.subs(z, x_2 - phi))))
